...
public class TestHelloHBase {
  ...
  public void testNamespaceExists() throws Exception {
    ...
    assertEquals("${symbol_pound}namespaceExists failed: found nonexistent namespace.",
            false, exists);

    admin.createNamespace
        (NamespaceDescriptor.create(EXISTING_NAMESPACE).build());
    exists = HelloHBase.namespaceExists(admin, EXISTING_NAMESPACE);
    assertEquals("${symbol_pound}namespaceExists failed: did NOT find existing namespace.",
            true, exists);
  ...
  public void testCreateNamespaceAndTable() throws Exception {
    ...
    assertEquals("${symbol_pound}createNamespaceAndTable failed to create namespace.",
            true, namespaceExists);

    boolean tableExists = admin.tableExists(HelloHBase.MY_TABLE_NAME);
    assertEquals("${symbol_pound}createNamespaceAndTable failed to create table.",
            true, tableExists);
  ...
  public void testPutRowToTable() throws IOException {
    ...
    assertEquals("${symbol_pound}putRowToTable failed to store row.", false, row.isEmpty());
  ...
  public void testDeleteRow() throws IOException {
    ...
    Result row = table.get(new Get(HelloHBase.MY_ROW_ID));
    assertEquals("${symbol_pound}deleteRow failed to delete row.", true, row.isEmpty());
