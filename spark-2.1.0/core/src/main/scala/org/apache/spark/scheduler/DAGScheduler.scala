...
class DAGScheduler(
  ...
  private[scheduler] def getShuffleDependencies(
    ...
    val visited = new HashSet[RDD[_]]// store the rdd that has been visited
    val waitingForVisit = new Stack[RDD[_]]// store the rdd that is to be visited
    waitingForVisit.push(rdd)
    while (waitingForVisit.nonEmpty) {
      val toVisit = waitingForVisit.pop()
      if (!visited(toVisit)) {
  ...
  //current --> parent stage,parent stage is created by getOrCreateShuffleMapStage
            ...
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                ...
                if (!mapStage.isAvailable) {
                  missing += mapStage// if wide dependicies add parent stage
                }
...
// we can regard this DAGSchedulerEventProcessLoop as DAGSchedular's interface
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  // receive various events , and handle here
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
    //handle job submitted object
